...
/**
 * cmd_exec - determines if a file is an executable command
 * @info: the info struct
 * @path: path to the file
 *
 * Return: 1 if true, 0 otherwise
 */
int cmd_exec(info_t *info, char *path)
{
	struct stat st;
...
char *find_cmd_path(info_t *info, char *pathstr, char *cmd)
{
	...
	while (1)
	{
		if (!pathstr[i] || pathstr[i] == ':')
		{
			...
			if (!*path)
				_string_concatenates(path, cmd);
			else
			{
				_string_concatenates(path, "/");
				_string_concatenates(path, cmd);
			}
			if (cmd_exec(info, path))
				return (path);
			if (!pathstr[i])
				break;
			curr_pos = i;
